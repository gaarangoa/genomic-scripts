...
@click.option('--lfile', required=True, help='File containing the length of the ARGs')
...
def deeparg_abundance(deeparg, lfile, ncounts, nlength, rlength, output_file, gfilter):
    '''
    ...
    fo1 = open(output_file+'.full.tsv', 'w')
    tree = []
    ...
    for antibiotic in arg_category:
        ...
        for arg in arg_category[antibiotic]:
            ...
            tree.append({'antibiotic_class': antibiotic, 'ARG': arg, 'relative_abundance': rel_abundances[arg]})

        log.debug(('SUM:', antibiotic, antibiotic_abn))
        fo.write( "\t".join([str(antibiotic), '-', str(antibiotic_abn)])+'\n' )
