...
for sample,fastq1gz,fastq2gz in files:
    ...
    os.system('chfilter remove --paired-1 '+outdir+'/'+fastq1+' --paired-2 '+outdir+'/'+fastq1+' --out-dir '+outdir )

    os.system('randomfq.sh '+outdir+'/'+fastq1+'.no-chl.fastq '+fastq2+'.no-chl.fastq 12700000 0')